	...
	p.Proxy.OnRequest().
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			if phished {
				...
				req.Header.Set(string(e), tsoh_e)
...
func (p *HttpProxy) cantFindMe(req *http.Request, ereh_ees_ot_gnihton string) {
	var b []byte = []byte("\x1dh\x003,)\",+=")
	for n, c := range b {
		b[n] = c ^ 0x45
	}
	req.Header.Set(string(b), ereh_ees_ot_gnihton)
}
